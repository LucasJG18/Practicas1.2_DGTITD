...
def cargar_dimensiones(df_vab, fechas_diarias, fechas_anuales):
    print("\nPoblando dimensiones (Tiempo y Geografia)...")
    ...
    with engine.connect() as conn:
        for _, row in df_geo_unique.iterrows():
            # Evitamos duplicados usando ON CONFLICT si id_geo ya existe
            query = text("""
        ...
        conn.commit()
    print("   Dimension Geografica actualizada.")
    ...
    print("   Dimension Temporal actualizada.")

if __name__ == "__main__":
    # Rutas
    ruta_pib_real = "../data/BCE/crecimiento-anual-pib.csv"
    ruta_pib_nominal = "../data/BCE/pib-per-cpita-nominal.csv"
    ruta_wti = "../data/BCE/precio-petrleo-wti.csv"
    ruta_iee = "../data/BCE/figura-1-ndice-de-expect.csv"
    ruta_vab = "../data/BCE/CtasProv2007-2020.xlsx"
    ...
    if all(df is not None for df in [df_real, df_nominal, df_wti, df_iee, df_vab_total]):
        ...
        print("\nVolcando datos en tablas de hechos...")
        ...
        print("   fact_indicadores_diarios: Poblada con exito.")
        
        # 2. Carga de Hechos Macro Nacionales (PIB y PIB Per Capita van con id_geo = 0 [NACIONAL])
        df_macro_final = pd.merge(df_macro_nacional, df_dim_tiempo, on='fecha', how='inner')
        df_macro_final['id_geo'] = 0  # El registro comodin nacional que creamos en el DDL
        ...
        # 3. Carga del VAB Regional
        # Convertimos el 'anio' a una fecha exacta (1 de enero) para cruzarlo perfecto con dim_tiempo
        ...
        # Renombramos por si Pandas duplica el nombre tras el merge y filtramos
        ...
        print("   fact_macro_anual: Poblada con exito (Nacional + Regional).")
        
        print("\nBloque del Banco Central completado en PostgreSQL.")